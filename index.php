...
<body link="#ffcba7"  alink="#5e216d" vlink="#ffcba7">
    <header>
        <p align="center">
          ЗОЛОТЫЕ
          <img align="center" src="source/handmade.png" style='height: 10%; width: 10%;'>
          РУЧКИ
        </p>
    </header>
    <?php
        ...
        else
        {
            ...
            {
                $state1 = $pdo->query('SELECT `имяКатегории` as `name` FROM `Категория`
                WHERE  `кодКатегории` = '.$_GET['red_id']);
                $row1 = $state1->fetch(PDO::FETCH_ASSOC);
                echo '<div width=100%><h2>'.$row1['name'].'</h2></div>';
                $state1 = $pdo->query('SELECT `кодТовара` as `tov` FROM `Товар`
