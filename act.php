...
<body link="#ffcba7"  alink="#5e216d" vlink="#ffcba7">
    ...
    <?php
        ...
        else
        {
            ...
            {
                $state11 = $pdo->query('SELECT * FROM `Подраздел` WHERE `кодПодраздела` = '.$_GET['red_id']);
                $row1 = $state11->fetch(PDO::FETCH_ASSOC);
                echo "<h2>".$row1["имяПодраздела"]."</h2>";
                $state0 = $pdo->query('SELECT `кодТовара` as `tov`, `имяПодраздела` as `name` FROM `Товар`, `Подраздел` WHERE `Товар`.`кодПодраздела` = '.$_GET['red_id'].' AND `Подраздел`.`кодПодраздела` = '.$_GET['red_id']);
