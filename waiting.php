...
<div id="MiniBlok" align="left" style="width: 74%;"><H2>Заказы, ожидающие обработки</H2>';
    $state1 = $pdo->query('SELECT * FROM `Пользователь` WHERE `слкод` = "'.$_GET['user'].'"');
    $row = $state1->fetch(PDO::FETCH_ASSOC);
$state1 = $pdo->query('SELECT * FROM `Заказ` WHERE `кодСтатуса` = 2 AND `кодПользователя` = '.$row['кодПользователя']);
$i = 0;
while($row1 = $state1->fetch(PDO::FETCH_ASSOC))
$i++;
if($i==0)
{
    echo "<h3>Пока что здесь пусто!</h3></div>";
}
else
{
    $state1 = $pdo->query('SELECT * FROM `Заказ` WHERE `кодСтатуса` = 2 AND `кодПользователя` = '.$row['кодПользователя']);
while($row1 = $state1->fetch(PDO::FETCH_ASSOC))
{
 ...
 echo '<p><table id="table"  width="100%">';  
