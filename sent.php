...
<div id="MiniBlok" align="left" style="width: 74%;"><H2>Отправленные заказы</H2>';/*
...
$state1 = $pdo->query('SELECT * FROM `Заказ` WHERE `кодСтатуса` = 5 AND `кодПользователя` = '.$row['кодПользователя']);
$i = 0;
while($row1 = $state1->fetch(PDO::FETCH_ASSOC))
$i++;
if ($i==0){
    echo '<h3>Здесь пока что пусто!</h3>';
}
else
{
    $state1 = $pdo->query('SELECT * FROM `Заказ` WHERE `кодСтатуса` = 5 AND `кодПользователя` = '.$row['кодПользователя']);
...
echo '</table>';
echo '<br><big><b>Трек-номер заказа: '.$row1['трек'].'</b></big>';
}
}
echo '
    </div>
